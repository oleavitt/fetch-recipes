    ...
    func load() async {
        ...
        if let cachedResponse = ImageLoader.cache.cachedResponse(for: URLRequest(url: url)),
           let cachedImage = UIImage(data: cachedResponse.data, scale: scale) {
            ...
            debugPrint("CachedAsyncImage: Cache hit (\(url.absoluteString))")
            return
        }
        ...
        do {
            ...
            if let image = UIImage(data: data, scale: scale) {
                ...
                debugPrint("CachedAsyncImage: Downloaded (\(url.absoluteString))")
                withAnimation(animation) {
                    phase = .success(Image(uiImage: image))
                }
            } else {
                debugPrint("CachedAsyncImage: Decode Error (\(url.absoluteString))")
                throw URLError(.cannotDecodeContentData)
            }

        } catch {
            debugPrint("CachedAsyncImage: Error (\(url.absoluteString))")
            withAnimation(animation) {
                phase = .failure(error)
            }
        }
